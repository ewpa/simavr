/*
 ...
 Converted from SPI to I2C by Ewan Parker.
...
ssd1306_start_i2c()
{
  i2c_start((SSD1306_I2C_ADDR<<1) + I2C_WRITE);
}

static inline void
ssd1306_tx_i2c_byte(const uint8_t byte)
{
  i2c_write(byte);
}
...
{
  ssd1306_tx_i2c_byte(byte);
}

void
ssd1306_write_instruction (const uint8_t byte)
{
  ssd1306_tx_i2c_byte(0x00);
  ssd1306_tx_i2c_byte(byte);
}
...
ssd1306_init_display (void)
{
  ...
  ssd1306_write_data (0x80);

  ssd1306_write_instruction (SSD1306_CHARGE_PUMP_REGULATOR);
  ssd1306_write_data (SSD1306_CHARGE_PUMP_ON);
  ssd1306_stop_i2c();
...
ssd1306_set_address_mode(const display_addr_t address_mode)
{
  ...
  ssd1306_write_data (address_mode);
  ssd1306_stop_i2c();
}
